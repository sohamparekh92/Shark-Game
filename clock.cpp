...
void Clock::draw() const { 
  ...
  //                                   secondsLoc[0], secondsLoc[1]);
    if(paused)
     m->drawMenu(0,0, IOManager::getInstance().getScreen());
...
void Clock::update() { 
 ...
 ticksSinceLastFrame = clockTicks - ticksAtLastFrame;
  int cappedRate = Gamedata::getInstance().getXmlInt("frameCap");
  bool capGate = Gamedata::getInstance().getXmlBool("framesAreCapped");
  if( capGate && ticksSinceLastFrame < (unsigned int)1000/ cappedRate)
  {
         SDL_Delay((1000/cappedRate) - ticksSinceLastFrame);
