...
Frame::Frame( const std::string& name, int n, SDL_Surface* surf ) : 
  screen(IOManager::getInstance().getScreen()),
  surface( surf ),
  width(Gamedata::getInstance().getXmlInt(name.substr(0,n)+"/width")), 
  height(Gamedata::getInstance().getXmlInt(name.substr(0,n)+"/height")),
  sourceX(0),
  sourceY(0)
{ }
