...
void Health::draw() const {
  ...
  Draw_AALine(screen, start[0], start[1], 
                      ...
                      thick, color);
  if(isDead)
   d->drawDeadMenu(0,0, IOManager::getInstance().getScreen());
  if(win)
   d->drawWinMenu(0,0, IOManager::getInstance().getScreen());
//  IOManager::getInstance().printMessageAt("DEAD!", 1000,70);
}
